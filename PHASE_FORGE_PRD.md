# Phase Forge PRD — Complete Build Specification
...
- Zoom controls (30%-300%) with localStorage persistence
- Dark theme (#262624 background, #da7757 orange accent, #e0e0e0 text)

**ALL existing functionality MUST be preserved.** The prompt injector becomes one part of the larger Phase Forge system.
...
A **separate** fixed sidebar panel, independent from the prompt injector.
...
The panel has **4 collapsible sections** in order. Each section header is clickable to expand/collapse (▶ collapsed, ▼ expanded). Sections 2-4 start **locked** (grayed out, 🔒 icon) until the previous section is complete.

---

## Feature 3: PRD Builder (Section 1)

### Mode Toggle
Pill toggle at the top: `[Questionnaire]` | `[Rant Mode]`
...
4. Unlock Section 2 (Configure Build)
...
- Mode toggle pill (Questionnaire / Rant)
- Start button
- NEXT button (appears after each Claude response)
...
## Feature 4: Build Configurator (Section 2)
...
These are prepended to each phase prompt when active:
...
- Locks Section 2 (shows values but grayed out)
- Unlocks Section 3 (Phases)
- Shows "Edit Config" button to re-unlock

---

## Feature 5: Phase Manager (Section 3)
...
- Drag-to-reorder (optional/nice-to-have, not required for v1)
...
## Feature 6: Phase Runner Engine (Section 4)
...
## Feature 7: All localStorage Keys
...
| `pf-prd-mode` | string | "questionnaire" or "rant" |
...
(function () {
  ...
  // Reactive state for PRD, config, phases, runner

  // ===== SECTION: PRD Builder Engine =====
  // Questionnaire prompts, rant prompts, step management, PRD capture
...
- **Branch:** `claude/fix-tampermonkey-prompt-2wj38`
- **Repo:** `/home/user/Greptacular`
...
3. Phase Forge panel toggles on/off via floating "PF" button
4. PRD Builder works in both Questionnaire and Rant mode
5. PRD is auto-captured when Claude outputs the markers
6. Configuration section calculates token budgets correctly
7. Phases can be imported manually or auto-generated
8. Phase Runner auto-sends phases, detects completion, advances to next
9. All state persists across page reloads via localStorage
10. Works on Claude.ai (primary), ChatGPT and Gemini (secondary)
