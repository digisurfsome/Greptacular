...
### Investigation 1: Application Map
...
6. Database type and connection — what database is used (Postgres, SQLite, etc.) and the environment variable for the connection string
7. Full schema — every table, columns, types, and relationships
8. Data flows per user action — for each user-facing action, document exactly what records are created, updated, or deleted and in which tables
9. Validation queries — for each data flow, provide the exact query to verify records are correct after the action
