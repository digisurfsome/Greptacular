...
**Phase 6: UI — PRD Creation Chat Panel (1-2 sessions)**
...
- `ui/src/hooks/useAgentOS.ts` (~150 lines) — React Query hooks for all Agent OS endpoints + WebSocket hook for interactive session.
- Integration with AppBuilderPage.tsx — Wire Agent OS panels into the existing page layout.

Why 1-2 sessions: UI is always harder to estimate. The chat panel alone could be one session. The auxiliary panels (standards, product, specs, gaps) could be another. Depends on complexity of the interactive flow.

Mini-PRD reference: Phase A step 5, Phase B step 5, Phase C step 6, Phase D step 6
...
| 6 | UI Components | ~850 | 1-2 |
| 7 | Expansion & Codebase Engine | ~650 | 1 |
| **Total** | | **~4,140** | **7-8 sessions** |
