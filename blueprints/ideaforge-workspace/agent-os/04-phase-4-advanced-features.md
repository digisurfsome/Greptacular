# Phase 4: Advanced Features — Agent OS Blueprint
...
Phase 4 is the final phase of the IdeaForge Workspace, adding six capability groups that transform the workspace from functional to production-grade:

1. **Chat Forking** -- branch from any message to explore tangents without polluting the original conversation.
2. **Inject from Another Chat** -- pull selected messages across conversations to cross-pollinate context.
3. **Export Chat to Markdown** -- download any conversation as a well-formatted `.md` file.
4. **Navigation Bar Integration** -- persistent breadcrumb navigation between the main AutoForge app and the workspace.
5. **Keyboard Shortcuts** -- workspace-specific shortcuts following the main app's established pattern.
6. **Polish & Edge Cases** -- empty states, loading skeletons, error recovery, responsive sidebar, input persistence, scroll behavior, and delete confirmation.

No new database tables are required. One new column (`forked_from_id`) is added to `workspace_conversations`.

### Target Users

Power users who want advanced conversation management, cross-pollination between chats, and a polished production-ready workspace experience. These users maintain multiple long-running conversations and need efficient navigation, branching, and export workflows.

### Core Use Cases

1. Fork a conversation from any message to explore tangents without polluting the original thread.
2. Inject selected messages from one conversation into another as prepended context, enabling knowledge transfer between topics.
3. Export any conversation as a well-formatted Markdown file for archival, sharing, or documentation.
4. Navigate seamlessly between the main AutoForge app and the workspace via persistent breadcrumb navigation.
5. Use keyboard shortcuts for rapid workspace navigation (new conversation, toggle panels, focus search, export).
6. Experience polished UX: empty states guide new users, loading skeletons prevent layout shift, smart scroll respects user position, drafts persist across conversation switches, and destructive actions require confirmation.

### Phase Context

- **Phase 1:** Core Chat Infrastructure (prerequisite -- provides conversations, messages, WebSocket, basic UI)
- **Phase 2:** Context Management & Organization (prerequisite -- provides categories, summaries, context budget, search)
- **Phase 3:** File Library & GitHub Integration (prerequisite -- provides library files, connected repos, file context)
- **Phase 4:** Advanced Features (this document -- final phase)
...
Six feature groups implemented across backend and frontend:

1. **Chat Forking** -- new column, backend fork logic, fork modal component
2. **Inject from Another Chat** -- backend inject endpoint, two-step selection modal, chat hook integration
3. **Export Chat to Markdown** -- backend markdown formatter, download endpoint, frontend trigger
4. **Navigation Bar Integration** -- breadcrumb nav bar in workspace, workspace link in main app header
5. **Keyboard Shortcuts** -- custom hook with modifier-key combos, help modal
6. **Polish & Edge Cases** -- eight subsystems covering empty states, loading, errors, responsive layout, auto-titles, delete confirmation, scroll behavior, and draft persistence
...
#### 1. `ui/src/components/workspace/ChatForkModal.tsx`
...
|                                      |
|  Select fork point:                  |
|  O [User] First message...       #1  |
|  O [Assistant] Response...       #2  |
|  O [User] Follow-up...          #3  |
...
#### 2. `ui/src/components/workspace/InjectFromChatModal.tsx`
...
#### 3. `ui/src/hooks/useWorkspaceKeyboardShortcuts.ts`
...
#### 4. `ui/src/components/workspace/WorkspaceKeyboardHelp.tsx`
...
**Add the `forked_from_id` column** to the `WorkspaceConversation` model and the migration function as described in the Database Changes section above.
...
#### 2. `server/routers/workspace.py`

Add four new endpoints to the existing workspace router.
...
#### 4. `ui/src/hooks/useWorkspaceChat.ts` -- Add Injection State
...
#### 5. `ui/src/pages/WorkspacePage.tsx` -- Integrate All Features
...
#### 6. `ui/src/App.tsx` -- Add Workspace Navigation Link
...
#### 7. `ui/src/lib/routes.ts` -- Already exists from Phase 1

Verify `isWorkspaceRoute()` exists. No changes needed if Phase 1 was implemented correctly.

#### 8. `ui/src/main.tsx` -- Already exists from Phase 1
...
**Props:** See Files to Create section 1.
...
**Props:** See Files to Create section 2.
