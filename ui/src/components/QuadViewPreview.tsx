...
export function QuadViewPreview({
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      ...
      // Each quadrant is half the container in each dimension
      const cellW = width / 2
    ...
    >
      ...
      >
        ...
        {viewMode === 'quad' ? (
          <div
            ref={quadGridRef}
            className="flex-1 overflow-hidden grid grid-cols-1 sm:grid-cols-2 gap-0"
          >
            {PAGES.map((page) => (
              <div
                key={page.id}
                className="relative overflow-hidden cursor-pointer group border-border"
                style={{
