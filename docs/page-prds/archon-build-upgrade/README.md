...
| File | Purpose |
|------|---------|
| [`PASS-0-PREAMBLE-AUDIT-HANDOFF.md`](./PASS-0-PREAMBLE-AUDIT-HANDOFF.md) | **Pass 0 — PREREQUISITE to everything else.** Handoff PRD for an agent to audit every `.archon/commands/stage-NN.md` prompt, strip `standalone-app` assumptions, and author per-stage preamble blocks with IF branches for all 6 build modes. Zero YAML changes. Opus 4.7 high. |
| [`PIPELINE-REBUILD-NO-BASH-HANDOFF.md`](./PIPELINE-REBUILD-NO-BASH-HANDOFF.md) | **Pass 1 — Rebuild handoff.** Single-file `prd-pipeline-d.yaml` with three preflight switches (model / mode / build-type). No bash. Consumes Pass 0 preambles. Covers all 6 build modes + full/prd-only/build-only via `@mode` tag. Opus 4.7 high. |
| [`ADD-NEW-BUILD-STYLE-HANDOFF.md`](./ADD-NEW-BUILD-STYLE-HANDOFF.md) | **Future-proof handoff.** Recipe for an agent to add a 7th (or Nth) build mode later. Because the pipeline was built mode-agnostic, adding a mode = authoring one new preamble branch per stage + one line in the `build-type-select` enum. No YAML rewrites, no core prompt edits. Run this AFTER Passes 0/1/2 are live. |
| [`MODEL-SELECTION-GUIDE.md`](./MODEL-SELECTION-GUIDE.md) | **Which Opus for which build.** Model naming translation table (opus-4.6-medium → claude-opus-4-6/medium, etc.), the three axes that drive the choice (complexity, novelty, cost-of-being-wrong), the scenario-to-model matrix, cost framing, and rules of thumb. Check this before every non-default build. |
| [`KNOWN-ISSUES.md`](./KNOWN-ISSUES.md) | **Open bugs and friction points.** Running log. Currently tracking: template second-copy bug (blocker for multi-module builds), bash variable-substitution bug (resolved by Pass 1 rebuild), graph-view stuck loading, three-phases-hardcoded. Add new issues here when you hit them. |
| [`ARCHON-VERSIONS-AND-REPOS.md`](./ARCHON-VERSIONS-AND-REPOS.md) | **Source-of-truth map of the 4 things.** Engine (`C:\Users\lober\archon\Archon\` v0.3.9 HEAD `83e6c7ac`, 2 days behind coleam00 latest), runtime install (`C:\Users\lober\.archon\`), template (`digisurfsome/archon-module-template`), working repo (`digisurfsome/archon-pipeline-rebuild` at `C:\Users\lober\archon-pipeline-rebuild\`). Read this when "wait, which one is the engine again?" comes up. |
