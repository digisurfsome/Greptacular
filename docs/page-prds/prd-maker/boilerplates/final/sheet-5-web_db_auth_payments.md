# Exact Reality Sheet 5: web_db_auth_payments (Full SaaS)

> **Variant**: Full SaaS -- everything active
> **Boilerplate**: DevToDollars Web-BoilerPlate-D2D (Next.js 16 + Supabase + Stripe + Tailwind v4 + PostHog)
> **What is active**: UI framework, styling, landing page, Supabase PostgreSQL, Supabase Auth, Stripe Payments, PostHog Analytics
> **What is dormant**: Nothing -- all boilerplate capabilities are wired and active
> **Use case**: Production SaaS application with user accounts, subscription billing, and analytics
>
...
| **Payments** | Stripe (checkout, webhooks, billing portal via Supabase Edge Functions) |
| **Analytics** | PostHog (page views, auth events, checkout events) |
...
nextjs/
  app/
    ...
    account/page.tsx        -- Account page (server component, auth-gated)
    auth/[id]/page.tsx      -- Auth pages (signin, signup, forgot_password, update_password, verify_email)
    api/
      auth_callback/route.ts   -- OAuth callback handler
      og/route.tsx             -- OG image generation
      reset_password/route.ts  -- Password reset callback
  components/
  ...
  utils/
    ...
    supabase/
      ...
      api.ts                -- Auth API client (signup, signin, oauth, signout, password reset)
      client.ts             -- Browser Supabase client
      middleware.ts          -- Session refresh middleware
      queries.ts            -- getUser, getSubscription, getProducts, getUserDetails
...
supabase/
  ...
  functions/
    _shared/
      ...
      stripe.ts             -- Stripe client, checkout, portal, webhook verification
      supabase.ts           -- Supabase admin client, upsert/delete helpers, customer management
      utils.ts              -- Shared utilities
    get_stripe_url/index.ts  -- Checkout session / billing portal URL
    stripe_webhook/index.ts  -- Stripe event processing
    on_user_modify/index.ts  -- PostHog events on user lifecycle
...
| S3 | Authentication provider | CRITICAL | Use the configured auth provider's sign-in flow with designated OAuth provider; restrict to approved sign-in methods | HANDLED | `nextjs/utils/supabase/api.ts` (email/password + Google + GitHub OAuth), `nextjs/components/misc/AuthForm.tsx` | None -- already implemented |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| AC1 | UserProfile interface with role | STANDARD | User profile includes `role` field with defined values (user, pro, admin) | MUST_BUILD | N/A -- `nextjs/schema.sql` `users` table has NO role column | Agent must: (1) Add `role text DEFAULT 'user' CHECK (role IN ('user', 'pro', 'admin'))` column to users table via migration, (2) Add RLS policy preventing users from modifying their own role, (3) Update `types_db.ts` |
| AC2 | Auth context provides full interface | STANDARD | Auth state provider exposes: user, profile, loading, sign-in/out, isAdmin, isPro | PARTIAL | `nextjs/utils/supabase/api.ts` provides auth functions. `nextjs/utils/supabase/queries.ts` has `getUser()` and `getUserDetails()`. No unified auth context with role booleans | Agent must create an auth context/hook that combines user, profile, loading state, and role convenience booleans (isAdmin, isPro). Currently auth state is fetched per-page via server components |
| AC3 | Profile created on first login | CRITICAL | On first auth where no DB profile exists, auto-create with default role and server timestamps | HANDLED | `nextjs/schema.sql` has `handle_new_user()` trigger that fires on `auth.users` insert, creating a `users` row with id, full_name, avatar_url from metadata | None -- already implemented via database trigger |
...
| AC6 | Popup/redirect sign-in flow | CRITICAL | Use auth provider's popup or redirect flow; catch errors with user-friendly feedback | HANDLED | `nextjs/utils/supabase/api.ts` `oauthSignin()` uses `signInWithOAuth` (redirect flow). `AuthForm.tsx` catches errors and shows toast | None -- already implemented |
| AC7 | Loading state during auth check | STANDARD | App shows loading state while initial auth check resolves; prevents flash of wrong content | PARTIAL | `nextjs/app/auth/[id]/page.tsx` checks auth server-side and redirects. `nextjs/utils/supabase/middleware.ts` refreshes session. But no global loading state visible during auth resolution | Agent should add a loading skeleton or spinner during initial auth resolution to prevent flash of unauthenticated content on protected pages |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| RG1 | ProtectedRoute for auth users | CRITICAL | Route guard checks auth state; redirects unauthenticated to login; spinner while loading | PARTIAL | `nextjs/app/account/page.tsx` checks auth server-side and redirects to `/auth/signin`. But no reusable ProtectedRoute component | Agent must create a reusable auth guard (either as middleware route matcher or a wrapper component) that protects all authenticated routes. Current pattern works per-page but is not DRY |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| DS1 | User data scoped to user | CRITICAL | All user-owned data scoped to authenticated user via RLS or user-specific paths | HANDLED | `nextjs/schema.sql` -- all tables with user data have RLS: `users` (auth.uid() = id), `subscriptions` (auth.uid() = user_id), `checkout_sessions` (auth.uid() = user_id). `customers` is private (no access) | None -- already implemented. Agent must maintain RLS on all new tables |
| DS2 | Helper for user data access | STANDARD | Utility function that abstracts database path/query construction for user-scoped data | PARTIAL | `nextjs/utils/supabase/queries.ts` provides `getUser()`, `getSubscription()`, `getProducts()`, `getUserDetails()` with React cache. But no generic user-scoped query helper | Agent should create a generic helper or ensure all new queries follow the pattern in `queries.ts`. RLS handles scoping automatically in Supabase |
...
| DS4 | Default sort newest first | POLISH | All list queries default to descending `createdAt` order | PARTIAL | `nextjs/utils/supabase/queries.ts` `getSubscription()` orders by `created` descending. `getProducts()` orders by `metadata->index`. No global default | Agent must ensure all new list queries use `order('created_at', { ascending: false })` by default |
...
| DSL2 | CRUD helper functions | STANDARD | Four base CRUD functions wrapping DB operations with automatic timestamp injection | MUST_BUILD | N/A -- no generic CRUD helpers exist. Only specific query functions in `queries.ts` | Agent must create generic CRUD helpers: `createRecord(table, data)`, `updateRecord(table, id, data)`, `deleteRecord(table, id)`, `getRecords(table, filters)` that auto-inject `updated_at` on writes |
...
| DSL4 | Delete account function | CRITICAL | Account deletion removes all user-owned data before removing user profile; cascading delete explicit | MUST_BUILD | N/A -- no account deletion function exists anywhere. `AccountPage.tsx` only has sign-out, no delete | Agent must build: (1) Account deletion UI with typed confirmation, (2) Server-side function that deletes from `checkout_sessions`, `subscriptions`, `customers`, `users` WHERE user_id matches, (3) Cancel Stripe subscription before deleting, (4) Call Supabase `auth.admin.deleteUser()` via edge function |
...
| RS2 | Public vs protected routes | CRITICAL | Landing and login are public; dashboard, profile, CRUD pages are protected | PARTIAL | `nextjs/app/page.tsx` (landing) is public. `nextjs/app/auth/` is public. `nextjs/app/account/page.tsx` checks auth and redirects. But no middleware-level route protection | Agent must add route protection either via Next.js middleware (checking auth on protected paths) or via a shared layout with auth check for `app/(protected)/` route group |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| DAP1 | Delete account removes all user data | CRITICAL | Iterate all user-data categories, delete all records, then delete user profile | MUST_BUILD | N/A | See DSL4. Agent must build cascading delete covering all user-owned tables |
...
| DAP6 | Default sort order | STANDARD | All lists default to descending createdAt (newest first) | PARTIAL | `getSubscription()` sorts by `created` desc | Agent must enforce this pattern on all new list queries |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| AS1 | Delete account requires typed confirmation | CRITICAL | User types "DELETE" to confirm; submit disabled until exact match | MUST_BUILD | N/A -- no account deletion exists | Agent must build: ConfirmModal with text input, submit disabled until input === 'DELETE' |
...
| AS4 | Protected routes wrap layout | STANDARD | All authenticated pages: RouteGuard > Layout > Page; public pages no auth wrapper | PARTIAL | Account page checks auth server-side. No consistent layout wrapping pattern | Agent should create `app/(protected)/layout.tsx` that checks auth for all nested routes |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| DBS1 | User data scoped to owner | CRITICAL | All user-owned data scoped via user-specific paths or RLS policies | HANDLED | All tables with user data have RLS enabled in `schema.sql` | None -- already implemented. Maintain for new tables |
| DBS2 | Delete cascades to all user data | CRITICAL | Account deletion removes all records in every user-data category BEFORE deleting user profile | MUST_BUILD | N/A | Agent must build cascading delete function |
...
| EH2 | Success feedback is toast + navigate | STANDARD | Every successful mutation: toast with message, then navigate to next logical view | PARTIAL | `AuthForm.tsx` shows toasts on auth success/error. `AccountPage.tsx` shows toast on sign-out and billing errors | Agent must ensure all new mutations follow this pattern |
| EH3 | Error feedback preserves form state | STANDARD | On error: show toast, stay on current view, do NOT clear form data | HANDLED | `AuthForm.tsx` preserves form state on error (email, password fields not cleared) | None -- pattern exists. Agent must follow it for all new forms |
...
| UX8 | Every action needs user feedback | POLISH | Every mutation shows success or error notification | PARTIAL | Auth actions show toasts. Billing actions show toasts | Agent must ensure all new mutations show toast feedback |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| PG1 | Replace service config placeholders | STANDARD | Replace all `YOUR_*` placeholders with real values | PARTIAL | `.env.example` has placeholders. Agent must copy to `.env.local` and fill in real values | Agent must provide instructions or prompt for: SUPABASE_URL, SUPABASE_ANON_KEY, STRIPE_SECRET_KEY, etc. |
...
| PG4 | Set data category names for delete | STANDARD | Update deletion handler with every user-data table | MUST_BUILD | N/A -- no deletion handler exists | Agent must build deletion handler with explicit table list |
...
| BI2 | Follow exact patterns | STANDARD | Use provided patterns for auth, theme, route guards, error boundary, service layer, UI | PARTIAL | Auth and theme patterns exist. Route guards, error boundary, and full service layer are missing | Agent must build missing patterns following Martin's specs |
...
| BI5 | Auth and data access working | CRITICAL | Sign-in, protected routes, CRUD through service layer, role-based access all functional end-to-end | PARTIAL | Sign-in works. Account page is auth-gated. No CRUD service layer. No role-based access | Agent must build service layer and role system |
...
| BI14 | Session expiry handling | CRITICAL | Catch auth expiry in data access; show "Session expired" toast; redirect to login | PARTIAL | `nextjs/utils/supabase/middleware.ts` refreshes session on each request. But no explicit expiry handling in client-side code | Agent must add session expiry detection in the API client layer and redirect to login with toast |
...
| BI16 | User avatar with fallback | POLISH | Avatar shows profile image with onError fallback to initials | PARTIAL | `nextjs/components/ui/avatar.tsx` exists (Radix Avatar with fallback). But not used in the app UI | Agent must use Avatar component in Navbar/AccountPage with initials fallback |
...
| MISC2 | No unprotected auth routes | CRITICAL | Every authenticated route must be wrapped in route guard | PARTIAL | Account page checks auth server-side. No middleware-level protection | Agent must add middleware or layout-level protection |
...
| MISC6 | User data scoped to owner | CRITICAL | All user data scoped via RLS | HANDLED | RLS enabled on all tables | None -- agent must maintain for new tables |
...
| BAN13 | No unprotected auth routes | CRITICAL | Route guards on all auth routes | PARTIAL | Account page checks auth but no reusable guard | Agent must add reusable protection |
...
| BAN34 | No unscoped user data | CRITICAL | RLS on all user data tables | HANDLED | RLS enabled | None -- agent must maintain for new tables |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| 208 | No hardcoded environment URLs | CRITICAL | All URLs from env vars; grep for http:// should only find env configs, tests, comments | HANDLED | `nextjs/utils/helpers.ts` uses `process.env.NEXT_PUBLIC_SITE_URL`. Supabase URL from `NEXT_PUBLIC_SUPABASE_URL`. Stripe URLs from Stripe SDK | None -- already implemented |
| 209 | Secrets never in source control | CRITICAL | No live secrets in repo; .env.example with placeholders; .gitignore blocks .env | HANDLED | `.env.example` has `YOUR_*` placeholders. `.gitignore` blocks `.env`. Stripe keys in Supabase secrets (not in repo) | None -- already implemented |
| 210 | Secrets never in client bundles | CRITICAL | No secret env vars in client-side code; only public identifiers | HANDLED | Client uses only `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_POSTHOG_*` (all public). Stripe secret key only in edge functions (Deno, server-side). Next.js `NEXT_PUBLIC_` prefix convention enforced | None -- already implemented |
...
| 216 | Same auth flow in all environments | CRITICAL | No mock auth in dev; same provider/flow as prod | HANDLED | Supabase local dev includes Auth emulator with same API contract. Same OAuth flow in dev and prod | None -- already implemented |
...
| 219 | Production data never in development | CRITICAL | No production data copies to dev; synthetic data only | HANDLED | Seed data is synthetic. `pnpm stripe:fixtures` loads test products. No production data sync scripts | None -- already implemented |
| 220 | Reproducible dev environment setup | STANDARD | Single setup command for new developers | PARTIAL | `supabase start` + `pnpm install` + `pnpm dev` + Stripe CLI setup. Documented in README. Not a single command but well-documented | Agent could create a `pnpm setup` script that chains all setup steps. Current multi-step documentation is acceptable |
...
| 227 | Client-side error reporting | STANDARD | Global error boundary catches uncaught exceptions; sends to error reporting endpoint | MUST_BUILD | N/A -- no error boundary, no error reporting | Agent must create error boundary + error reporting (could use PostHog for errors) |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| 236 | Privacy policy page | CRITICAL | `/privacy` route with data collection, storage, sharing, retention, deletion info. Linked from signup and footer | MUST_BUILD | N/A -- no privacy policy page. **CRITICAL gap with Stripe active -- Stripe TOS requires a privacy policy** | Agent must create `app/privacy/page.tsx` with comprehensive privacy policy. Link from Footer and signup flow |
| 237 | Terms of service page | STANDARD | `/terms` route with acceptable use, termination, liability, jurisdiction. Checkbox on signup. Acceptance timestamp stored | MUST_BUILD | N/A -- no terms of service page | Agent must create `app/terms/page.tsx`. Add acceptance checkbox to AuthForm signup. Store acceptance timestamp in users table |
| 238 | Cookie consent mechanism | CRITICAL | Consent banner before non-essential cookies. Accept/Reject buttons. Cookie preferences respected | MUST_BUILD | N/A -- **PostHog and Stripe set cookies without consent. GDPR violation** | Agent must build cookie consent banner. Block PostHog script until consent given. Only essential cookies (auth session) without consent |
| 239 | Data export capability | STANDARD | User can export their personal data as JSON/CSV download | MUST_BUILD | N/A | Agent must build: button in account settings that generates downloadable JSON of all user data (profile, subscriptions, checkout_sessions) |
| 240 | Data deletion capability | CRITICAL | User can delete account and all data. Deletion within 30 days. Retain only legally required data | MUST_BUILD | N/A -- **no account deletion exists. GDPR Article 17 violation with user data stored** | Agent must build account deletion flow: typed confirmation > delete user data from all tables > cancel Stripe subscription > delete auth user > sign out |
| 241 | Consent tracking | STANDARD | `user_consents` table storing user_id, consent_type, document_version, timestamp, IP | MUST_BUILD | N/A | Agent must create `user_consents` table via migration and record consent on signup and cookie preferences |
| 242 | Third-party data sharing disclosure | STANDARD | Privacy policy lists every third party receiving data: Stripe, PostHog, Supabase, Loops | MUST_BUILD | N/A -- **sharing data with Stripe (payments), PostHog (analytics), Supabase (backend) without disclosure** | Agent must include third-party disclosure section in privacy policy |
...
| 258 | API response envelope consistency | STANDARD | Consistent response structure across all endpoints | PARTIAL | Edge functions return `{ redirect_url }`. Stripe webhook returns `{ received: true }`. No consistent envelope | If agent builds API routes, must use consistent envelope: `{ data, error }` |
...
| # | Rule | Severity | Martin's Spec | Status | Boilerplate Location | Agent Action Required |
|---|------|----------|--------------|--------|---------------------|----------------------|
| 264 | Transient failure detection | STANDARD | Classify 502/503/504/429 as transient (retry); 400/401/403/404/422 as permanent (no retry) | PARTIAL | Edge functions have try/catch. `get_stripe_url` handles errors. But no systematic classification | Agent must create an HTTP client utility that classifies errors as transient vs permanent |
...
| 267 | Circuit breaker for external services | POLISH | After N consecutive failures, stop sending requests; cooldown; half-open test | MUST_BUILD | N/A | Agent should implement for Stripe API calls if reliability is a concern |
| 268 | Graceful degradation | STANDARD | Critical vs non-critical service classification; non-critical shows fallback, not crash | PARTIAL | If Stripe is down, pricing still renders from DB cache. Auth failure redirects cleanly. But no formal classification | Agent should classify services and implement fallback states |
...
| 270 | Idempotency for retried operations | CRITICAL | Write operations safe to execute multiple times; idempotency keys for payments | PARTIAL | Stripe SDK handles idempotency for checkout sessions internally. Webhook handler uses Stripe event IDs. But no app-level idempotency keys | Agent must add idempotency keys for any non-Stripe write operations that might be retried |
...
|----|----------|--------|-----------|
| A | Data Input | MUST_BUILD | Forms exist for auth only. App-specific input patterns (forms, file upload, etc.) must be built |
| B | Data Storage | HANDLED | Supabase PostgreSQL configured with tables, RLS, migrations, seed data |
...
| D | Data Output | PARTIAL | Landing page sections (Hero, Pricing, FAQ, Stats). No list views, detail views, charts, or dashboards |
| E | Authentication | HANDLED | Email/password + Google/GitHub OAuth + email verification + password reset + session management |
| F | Authorization | PARTIAL | RLS policies on all tables. But no role column, no role-based route guards, no feature gating by subscription tier |
| G | Communication | MUST_BUILD | Loops.so referenced but not integrated in Next.js. No email notifications, no in-app notifications |
| H | Integration | HANDLED | Stripe fully connected (checkout, webhooks, billing portal). PostHog connected (page views, auth events, checkout events) |
| I | Workflow | MUST_BUILD | No workflow engine, state machines, or automation. Only DB trigger for user profile creation |
...
| L | Monetization | HANDLED | Stripe checkout, webhook sync, subscription tracking, billing portal. Supports one-time and recurring payments |
| M | Admin/Ops | MUST_BUILD | No admin panel, no user management, no content moderation |
| N | Infrastructure | HANDLED | Supabase (DB + Auth + Edge Functions + Realtime), Stripe (payments), PostHog (analytics), Vercel (hosting). CI/CD via GitHub Actions (Flutter web only -- Next.js CI missing) |
...
**Miscellaneous**: MISC3, MISC6, MISC9, MISC10, MISC14, MISC16
**Banned Patterns**: BAN1, BAN2, BAN3, BAN6, BAN7, BAN8, BAN9, BAN10, BAN14, BAN15, BAN16, BAN34
**Industry Standards**: 208, 209, 210, 213, 215, 216, 217, 218, 219, 226, 228, 229, 230, 231, 233, 248, 253
**Mechanism Categories**: B, E, H, L, N

**Total HANDLED: ~72 rules**
...
| DSL4 | Delete account function | Build edge function or server action for complete account deletion |
| AS1 | Delete with typed confirmation | Build ConfirmModal with text input disabled until "DELETE" typed |
...
| DBS2 | Delete cascades all data | Build ordered deletion across all user-data tables |
...
| 227 | Client error reporting | Create error boundary + reporting to PostHog |
...
| 236 | Privacy policy page | **CRITICAL**: Create `/privacy` route. Required by Stripe TOS and GDPR |
| 237 | Terms of service page | Create `/terms` route with acceptance tracking |
| 238 | Cookie consent | **CRITICAL**: Build consent banner. Block PostHog until consent given |
| 239 | Data export | Build user data export as JSON download |
| 240 | Data deletion | **CRITICAL**: Build complete account deletion flow |
| 241 | Consent tracking | Create `user_consents` table and record all consent events |
| 242 | Third-party data disclosure | Document Stripe, PostHog, Supabase in privacy policy |
...
| 267 | Circuit breaker | Implement for Stripe API calls |
| 269 | Retry state visible | Show "Retrying..." UI during transient failures |

**Total MUST_BUILD: ~80 rules**
...
| Banned (HANDLED) | ~18 | ~7% |
| Banned (PARTIAL) | ~8 | ~3% |
| Banned (MUST_BUILD) | ~31 | ~12% |

### Critical Items Still Missing (MUST FIX for Production)

1. **Privacy policy page (#236)** -- Stripe TOS requires it. GDPR requires it. Legal violation to accept payments without one
2. **Cookie consent mechanism (#238)** -- PostHog + Stripe cookies without consent = GDPR violation
3. **Data deletion capability (#240)** -- GDPR Article 17 right to erasure. Users have payment data and cannot delete accounts
...
9. **Idempotency for retried operations (#270)** -- No app-level idempotency keys
10. **Dependency security audit in CI (#232)** -- No CI for the Next.js app at all
11. **Open-source license compliance (#243)** -- No THIRD_PARTY_LICENSES file or audit
12. **Form validation (#MISC8, BAN35)** -- Auth forms validate passwords but no per-field inline validation

---

## What an Agent Needs to Know

1. **Everything is live** -- DB, Auth, Payments, Analytics all connected and functional
2. **The boilerplate is a LANDING PAGE with auth and billing** -- it is NOT an app. There are no CRUD features, no dashboard, no admin panel, no sidebar layout. The agent builds the actual application on top of this foundation
3. **Stripe requires setup**: Create Stripe account, configure products/prices, set `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` in Supabase secrets, run `pnpm stripe:listen` for local dev
4. **PostHog requires setup**: Set `NEXT_PUBLIC_POSTHOG_KEY` and `NEXT_PUBLIC_POSTHOG_HOST` in `.env`
5. **Webhook sync is automatic** -- Stripe events sync to DB without app intervention
6. **Legal compliance is the BIGGEST gap** -- privacy policy, terms, cookie consent, data deletion are ALL missing. These are required for a production SaaS and must be the first things built
7. **No role/tier enforcement** -- Stripe knows who is subscribed, but the app has no role column and does not gate features by subscription tier. Must build: role column + subscription check + route guards
8. **No email notifications** -- Loops.so referenced in edge functions but not integrated into the Next.js app. Auth emails come from Supabase. No transactional email
9. **The "service layer" is `utils/supabase/queries.ts`** -- it is minimal (4 functions). Components like Pricing.tsx bypass it and call Supabase directly. Agent must enforce service layer discipline
10. **Next.js App Router conventions** -- this boilerplate uses the App Router (not Pages Router). Routes are folders in `app/`. Server Components are default. Client Components need `'use client'` directive. Middleware in `middleware.ts` at project root
