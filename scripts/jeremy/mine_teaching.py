#!/usr/bin/env python3
"""
mine_teaching.py — Jeremy Miner NEPQ Teaching Content Miner

Walks 5 specific teaching transcript .md files, splits by section/video,
chunks into 4 000-token windows with 500-token overlap, calls Claude Sonnet
via subscription (claude -p) to extract verbatim TEACHING content.

Extracts: framework_definition, gate_criteria, tone_note, nepq_question_taught,
          example_phrase_taught, mistake_warning, transition_taught, reframe_taught

Output:
    docs/info/jeremy-miner-corpus/teaching-corpus/
    ├── stage-1-connecting.md
    ├── stage-2-situation.md
    ├── stage-3-problem-awareness.md
    ├── stage-4-solution-awareness.md
    ├── stage-5-consequence.md
    ├── stage-6-transition-presentation.md
    ├── stage-7-commitment.md
    ├── gate-criteria.md
    ├── tone-notes.md
    ├── mistakes-warnings.md
    ├── reframes-taught.md
    └── _teaching-index.md

Fully resumable — kill at any time, restart and it skips done chunks.
Progress: teaching-corpus/_teaching_mine_progress.json

Usage:
    python mine_teaching.py
    python mine_teaching.py --limit 5      # smoke test on first 5 sections
    python mine_teaching.py --dry-run      # print chunks only, no API calls

Requirements:
    pip install tiktoken
    claude login  (subscription — no API key needed)
"""

import argparse
import asyncio
import json
import os
import re
import shutil
import sys
import tempfile
from datetime import datetime
from pathlib import Path

# ── Pop CLAUDECODE at import time so every subprocess inherits clean env ───────
os.environ.pop("CLAUDECODE", None)

# ── Dependency check ──────────────────────────────────────────────────────────
try:
    import tiktoken
except ImportError:
    print("ERROR: tiktoken not installed.  Run: pip install tiktoken")
    sys.exit(1)


# ── Load .env (no python-dotenv required) ────────────────────────────────────
def _load_dotenv() -> None:
    for candidate in [
        Path(".env"),
        Path(__file__).parent / ".env",
        Path(__file__).parent.parent.parent / ".env",
    ]:
        if candidate.exists():
            for line in candidate.read_text(encoding="utf-8").splitlines():
                line = line.strip()
                if line and not line.startswith("#") and "=" in line:
                    k, _, v = line.partition("=")
                    os.environ.setdefault(k.strip(), v.strip().strip('"').strip("'"))
            break


_load_dotenv()


# ══════════════════════════════════════════════════════════════════════════════
# CONFIGURATION
# ══════════════════════════════════════════════════════════════════════════════

# Repo root — script is at scripts/jeremy/mine_teaching.py → 2 parents up
REPO_ROOT = Path(__file__).parent.parent.parent

# 5 target teaching transcript files (relative to repo root)
TARGET_FILES = [
    REPO_ROOT / "docs/info/jeremy-miner-corpus/videos/01-Intro to NEPQ Selling/all-transcripts.md",
    REPO_ROOT / "docs/info/jeremy-miner-corpus/videos/02-The Power of NEPQ/all-transcripts.md",
    REPO_ROOT / "docs/info/jeremy-miner-corpus/videos/02-The NEPQ Kickstart Crash Course/all-transcripts.md",
    REPO_ROOT / "docs/info/jeremy-miner-corpus/videos/Jeremy Miner - NEPQ Black Book Bundle/all-transcripts.md",
    REPO_ROOT / "docs/info/jeremy-miner-corpus/videos/The Training/all-transcripts.md",
]

OUTPUT_DIR      = REPO_ROOT / "docs/info/jeremy-miner-corpus/teaching-corpus"
PROGRESS_FILE   = OUTPUT_DIR / "_teaching_mine_progress.json"
ERROR_LOG_FILE  = OUTPUT_DIR / "_teaching_mine_errors.log"

MODEL           = "claude-sonnet-4-5-20250929"   # Sonnet 4.6
CHUNK_TOKENS    = 4000
OVERLAP_TOKENS  = 500
MAX_CONCURRENCY = 5
CALL_TIMEOUT    = 120        # seconds per subprocess call
RETRY_DELAYS    = [2, 5, 10] # backoff on exit 15 / exit 3

# ── NEPQ stage metadata ───────────────────────────────────────────────────────
STAGE_FILES = {
    0: "stage-0-general.md",          # general / not stage-specific
    1: "stage-1-connecting.md",
    2: "stage-2-situation.md",
    3: "stage-3-problem-awareness.md",
    4: "stage-4-solution-awareness.md",
    5: "stage-5-consequence.md",
    6: "stage-6-transition-presentation.md",
    7: "stage-7-commitment.md",
}

STAGE_NAMES = {
    0: "General",
    1: "Connect",
    2: "Situation",
    3: "Problem Awareness",
    4: "Solution Awareness",
    5: "Consequence",
    6: "Transition / Presentation",
    7: "Commitment",
}

TYPE_FILES = {
    "gate_criteria":       "gate-criteria.md",
    "tone_note":           "tone-notes.md",
    "mistake_warning":     "mistakes-warnings.md",
    "reframe_taught":      "reframes-taught.md",
}

ALL_TYPES = [
    "framework_definition",
    "gate_criteria",
    "tone_note",
    "nepq_question_taught",
    "example_phrase_taught",
    "mistake_warning",
    "transition_taught",
    "reframe_taught",
]

# Resolved at runtime
CLAUDE_CLI: str = ""


# ══════════════════════════════════════════════════════════════════════════════
# SYSTEM PROMPT  (verbatim from spec)
# ══════════════════════════════════════════════════════════════════════════════

SYSTEM_PROMPT = """\
Extract verbatim TEACHING content from Jeremy Miner NEPQ training transcripts. \
Focus on framework/structure, not deployed sales lines.

EXTRACT:
- framework_definition — Jeremy defining a stage / concept / principle
- gate_criteria — explicit description of when to advance from stage X to X+1
- tone_note — delivery instruction (pace, pause, tonality, "say this softly")
- nepq_question_taught — verbatim Q he teaches at any stage
- example_phrase_taught — verbatim example phrase he gives as illustration
- mistake_warning — verbatim "don't do this" instruction
- transition_taught — bridge phrasing he teaches between stages
- reframe_taught — example of polarity rephrase

DO NOT extract:
- Real deployed sales lines from role-plays (Agent 3 handles those)
- Specific named objection handlers (Agent 4 handles)
- Generic motivation/mindset (Agent 5 handles)

NEPQ 7 stages: 1-Connect, 2-Situation, 3-Problem-Aware, 4-Solution-Aware, \
5-Consequence, 6-Transition/Present, 7-Commit. \
Use 0 for general / not stage-specific content.

OUTPUT (strict JSON, nothing outside the JSON object):
{"extractions":[{"text":"verbatim","type":"framework_definition","nepq_stage":3,\
"context_note":"brief","tags":["tag1","tag2"]}]}

RULES:
- VERBATIM ONLY. Every word in "text" must appear in the chunk exactly as written.
- Do not paraphrase, reconstruct, or summarise. If you cannot find exact words, skip it.
- nepq_stage must be an integer 0-7.
- type must be one of the 8 types listed above (exact spelling).
- tags should be 1-3 short lowercase keywords (e.g. "pacing", "stage-gate", "tonality").
- Return {"extractions":[]} if no teaching content found in this chunk.
- No markdown fences, no explanation — JSON object only.\
"""


# ══════════════════════════════════════════════════════════════════════════════
# HELPERS
# ══════════════════════════════════════════════════════════════════════════════

def resolve_claude_cli() -> str:
    cli = shutil.which("claude")
    if not cli:
        print("FATAL: 'claude' CLI not found on PATH.")
        print("       Install Claude Code: https://docs.anthropic.com/en/docs/claude-code")
        sys.exit(2)
    return cli


def setup_scratch_dir() -> str:
    scratch = tempfile.mkdtemp(prefix="teach_miner_")
    settings = Path(scratch) / ".claude-teach-miner-settings.json"
    settings.write_text(json.dumps({
        "permissions": {"defaultMode": "acceptEdits", "allow": []},
    }), encoding="utf-8")
    return scratch


def split_sections(text: str, folder_name: str) -> list[tuple[str, str]]:
    """Split an all-transcripts.md into (video_title, content) pairs.

    Sections are delimited by `---` lines. Section titles are `## heading` lines.
    """
    sections: list[tuple[str, str]] = []
    current_title = folder_name
    current_lines: list[str] = []

    for line in text.splitlines():
        stripped = line.strip()
        if stripped.startswith("## "):
            # Flush current section
            body = "\n".join(current_lines).strip()
            if body:
                sections.append((current_title, body))
            current_title = stripped[3:].strip()
            current_lines = []
        elif stripped == "---":
            # Section separator — flush but DON'T reset title (next ## will do it)
            body = "\n".join(current_lines).strip()
            if body:
                sections.append((current_title, body))
            current_lines = []
        elif stripped.startswith("# ") and not sections and not current_lines:
            # Top-level title — use as initial context
            current_title = stripped[2:].strip()
        else:
            current_lines.append(line)

    # Flush last section
    body = "\n".join(current_lines).strip()
    if body:
        sections.append((current_title, body))

    return sections


def chunk_text(text: str, enc) -> list[str]:
    """Split into CHUNK_TOKENS windows with OVERLAP_TOKENS overlap."""
    tokens = enc.encode(text)
    chunks: list[str] = []
    start = 0
    while start < len(tokens):
        end = min(start + CHUNK_TOKENS, len(tokens))
        chunks.append(enc.decode(tokens[start:end]))
        if end >= len(tokens):
            break
        start = end - OVERLAP_TOKENS
    return chunks


def load_state(path: Path) -> dict:
    if path.exists():
        try:
            data = json.loads(path.read_text(encoding="utf-8"))
            data.setdefault("processed", {})
            data.setdefault("call_count", 0)
            data.setdefault("extractions_total", 0)
            return data
        except Exception as e:
            print(f"Warning: progress file unreadable ({e}) — starting fresh.")
    return {"processed": {}, "call_count": 0, "extractions_total": 0}


def save_state(path: Path, state: dict) -> None:
    path.write_text(json.dumps(state, indent=2, ensure_ascii=False), encoding="utf-8")


def log_error(msg: str) -> None:
    ts = datetime.now().isoformat(timespec="seconds")
    entry = f"[{ts}]  {msg}\n"
    print(f"  ⚠  {entry.strip()}", flush=True)
    with open(ERROR_LOG_FILE, "a", encoding="utf-8") as fh:
        fh.write(entry)


# ══════════════════════════════════════════════════════════════════════════════
# PREFLIGHT
# ══════════════════════════════════════════════════════════════════════════════

async def preflight_auth_check() -> None:
    env = os.environ.copy()
    env.pop("CLAUDECODE", None)
    env.pop("ANTHROPIC_API_KEY", None)

    print("Preflight: verifying claude subscription auth …", flush=True)
    try:
        proc = await asyncio.create_subprocess_exec(
            CLAUDE_CLI, "-p", "say only the word OK",
            "--model", MODEL,
            "--output-format", "text",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            env=env,
        )
        out_b, err_b = await asyncio.wait_for(proc.communicate(), timeout=60)
        out = out_b.decode("utf-8", errors="replace").strip()
        err = err_b.decode("utf-8", errors="replace").strip()

        if proc.returncode != 0:
            print(f"\nFATAL: preflight failed — exit {proc.returncode}")
            print(f"  stderr: {err[:300] if err else '(empty)'}")
            print(f"  stdout: {out[:300] if out else '(empty)'}")
            print("\nFix: run `claude login` then retry.")
            sys.exit(3)

        print(f"Preflight OK — claude says: {out[:60]!r}\n", flush=True)

    except asyncio.TimeoutError:
        print("FATAL: preflight timed out. Auth prompt likely hanging — run `claude login`.")
        sys.exit(3)
    except FileNotFoundError as e:
        print(f"FATAL: cannot launch claude CLI: {e}")
        sys.exit(3)


# ══════════════════════════════════════════════════════════════════════════════
# API CALL  (subscription via claude -p subprocess)
# ══════════════════════════════════════════════════════════════════════════════

async def call_claude(
    user_msg: str,
    source_label: str,
    chunk_idx: int,
) -> list[dict]:
    """Call Claude via subscription. Returns list of extraction dicts."""
    env = os.environ.copy()
    env.pop("CLAUDECODE", None)
    env.pop("ANTHROPIC_API_KEY", None)

    combined_prompt = (
        "SYSTEM INSTRUCTIONS:\n"
        + SYSTEM_PROMPT
        + "\n\n---\n\nTRANSCRIPT CHUNK TO ANALYZE:\n"
        + user_msg
    )

    for attempt, delay in enumerate([0] + RETRY_DELAYS):
        if delay:
            await asyncio.sleep(delay)

        try:
            proc = await asyncio.create_subprocess_exec(
                CLAUDE_CLI, "-p", combined_prompt,
                "--model", MODEL,
                "--output-format", "text",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                env=env,
            )
            try:
                out_b, err_b = await asyncio.wait_for(
                    proc.communicate(), timeout=CALL_TIMEOUT
                )
            except asyncio.TimeoutError:
                proc.kill()
                log_error(f"{source_label}  chunk={chunk_idx}  TimeoutExpired after {CALL_TIMEOUT}s")
                if attempt < len(RETRY_DELAYS):
                    continue
                return []

            raw_out = out_b.decode("utf-8", errors="replace").strip()
            raw_err = err_b.decode("utf-8", errors="replace").strip()

            # First-chunk debug peek
            if chunk_idx == 0 and attempt == 0:
                preview = raw_out[:200].replace("\n", " ")
                print(f"  [DEBUG chunk0]: {preview!r}", flush=True)

            if proc.returncode != 0:
                # exit 15 / exit 3 = spurious SIGTERM → retry
                if proc.returncode in (15, 3) and attempt < len(RETRY_DELAYS):
                    log_error(f"{source_label}  chunk={chunk_idx}  exit {proc.returncode} — retrying")
                    continue
                log_error(f"{source_label}  chunk={chunk_idx}  exit {proc.returncode}: {raw_err[:200]}")
                return []

            if not raw_out:
                return []

            # Strip markdown fences if model wrapped the JSON
            m = re.search(r'\{[\s\S]*\}', raw_out)
            if m:
                raw_out = m.group(0)

            parsed = json.loads(raw_out)
            raw_items = parsed.get("extractions", [])

            extractions: list[dict] = []
            for item in raw_items:
                if not isinstance(item, dict):
                    continue
                text = item.get("text", "").strip()
                etype = item.get("type", "")
                if not text or etype not in ALL_TYPES:
                    continue
                stage = item.get("nepq_stage", 0)
                if not isinstance(stage, int) or stage < 0 or stage > 7:
                    stage = 0
                extractions.append({
                    "text":         text,
                    "type":         etype,
                    "nepq_stage":   stage,
                    "context_note": item.get("context_note", ""),
                    "tags":         item.get("tags", []),
                })

            return extractions

        except json.JSONDecodeError as e:
            log_error(f"{source_label}  chunk={chunk_idx}  JSON parse failed: {e}  raw: {raw_out[:150]}")
            return []
        except Exception as e:
            log_error(f"{source_label}  chunk={chunk_idx}  {type(e).__name__}: {e}")
            return []

    return []


# ══════════════════════════════════════════════════════════════════════════════
# PIPELINE
# ══════════════════════════════════════════════════════════════════════════════

async def run(limit: int = 0, dry_run: bool = False) -> None:
    global CLAUDE_CLI

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    sep = "─" * 66
    print(f"\n{sep}")
    print("  Jeremy Miner NEPQ Teaching Content Miner")
    print("  Auth:    subscription (claude -p)")
    print(f"  Output:  {OUTPUT_DIR}")
    print(f"  Progress:{PROGRESS_FILE}")
    print(f"  Concurrency: {MAX_CONCURRENCY}  |  Chunk: {CHUNK_TOKENS}tok  |  Overlap: {OVERLAP_TOKENS}tok")
    print(f"{sep}\n")

    # Resolve CLI
    CLAUDE_CLI = resolve_claude_cli()
    print(f"Claude CLI: {CLAUDE_CLI}\n", flush=True)

    if not dry_run:
        await preflight_auth_check()

    # Load resume state
    state = load_state(PROGRESS_FILE)
    print(f"Loaded progress: {len(state['processed'])} chunks done | "
          f"{state['call_count']} calls | {state['extractions_total']} extractions so far\n")

    # Verify target files exist
    missing = [f for f in TARGET_FILES if not f.exists()]
    if missing:
        print("ERROR: missing target files:")
        for m in missing:
            print(f"  {m}")
        sys.exit(1)

    # Tokeniser
    try:
        enc = tiktoken.get_encoding("cl100k_base")
    except Exception:
        enc = tiktoken.get_encoding("gpt2")

    # ── Build work queue ───────────────────────────────────────────────────────
    # WorkItem: (folder_name, video_title, chunk_idx, total_chunks, chunk_text, key)
    work_queue: list[tuple] = []
    section_count = 0

    for target_path in TARGET_FILES:
        folder_name = target_path.parent.name
        try:
            raw = target_path.read_text(encoding="utf-8", errors="replace")
        except Exception as e:
            log_error(f"Read error {target_path}: {e}")
            continue

        sections = split_sections(raw, folder_name)
        print(f"{folder_name}: {len(sections)} sections", flush=True)

        for video_title, section_text in sections:
            section_count += 1
            if limit > 0 and section_count > limit:
                break

            chunks = chunk_text(section_text, enc) if section_text else []
            if not chunks:
                continue

            total = len(chunks)
            for chunk_idx, ck in enumerate(chunks):
                key = f"{folder_name}::{video_title}::chunk_{chunk_idx}"
                if key not in state["processed"]:
                    work_queue.append((folder_name, video_title, chunk_idx, total, ck, key))

        if limit > 0 and section_count > limit:
            break

    total_new = len(work_queue)
    already_done = len(state["processed"])
    print(f"\nChunks to process: {total_new}  |  Already done: {already_done}\n")

    if dry_run:
        print("=== DRY RUN — first 5 chunks ===")
        for item in work_queue[:5]:
            folder, video, cidx, total, ck, key = item
            print(f"\n  [{folder}]  {video!r}  chunk {cidx+1}/{total}")
            print(f"  Preview: {ck[:200].replace(chr(10), ' ')!r}")
        print("\nDry run done. No API calls made.")
        return

    # ── Dispatch ───────────────────────────────────────────────────────────────
    if total_new == 0:
        print("Nothing new — all chunks done.\n")
    else:
        semaphore = asyncio.Semaphore(MAX_CONCURRENCY)
        lock = asyncio.Lock()

        async def process_one(item: tuple) -> None:
            folder_name, video_title, chunk_idx, total_chunks, ck, key = item

            source_label = f"[{folder_name[:30]}] {video_title[:25]}"

            user_msg = (
                f"Source folder: {folder_name}\n"
                f"Video / section: {video_title}\n"
                f"Chunk {chunk_idx + 1} of {total_chunks}:\n\n"
                f"{ck}\n\n"
                "Extract teaching content. JSON only."
            )

            async with semaphore:
                extractions = await call_claude(user_msg, source_label, chunk_idx)

            # Annotate with provenance
            for ex in extractions:
                ex["source_folder"] = folder_name
                ex["source_video"]  = video_title

            async with lock:
                state["processed"][key] = {
                    "extractions": extractions,
                    "ts": datetime.now().isoformat(timespec="seconds"),
                }
                state["call_count"]       += 1
                state["extractions_total"] += len(extractions)
                save_state(PROGRESS_FILE, state)

            tag = f"  → {len(extractions)} extraction(s) ✓" if extractions else ""
            print(
                f"  [{folder_name[:28]:<28}]  {video_title[:28]:<28}  "
                f"chunk {chunk_idx+1:>2}/{total_chunks}{tag}",
                flush=True,
            )
            for ex in extractions:
                stage_label = STAGE_NAMES.get(ex.get("nepq_stage", 0), "?")
                preview = ex.get("text", "")[:90].replace("\n", " ")
                print(
                    f"     ┌─ [{ex['type']}]  stage:{ex['nepq_stage']}-{stage_label}",
                    flush=True,
                )
                print(f"     └─ \"{preview}…\"", flush=True)

        tasks    = [asyncio.create_task(process_one(item)) for item in work_queue]
        done_cnt = 0
        start_ts = asyncio.get_event_loop().time()

        for fut in asyncio.as_completed(tasks):
            await fut
            done_cnt += 1
            if done_cnt % 20 == 0 or done_cnt == total_new:
                elapsed   = asyncio.get_event_loop().time() - start_ts
                rate      = done_cnt / elapsed if elapsed > 0 else 0
                remaining = (total_new - done_cnt) / rate if rate > 0 else 0
                print(
                    f"\n  ── {done_cnt}/{total_new} ({done_cnt/total_new*100:.0f}%)"
                    f"  {rate:.1f} chunks/s"
                    f"  ETA: {remaining/60:.1f}min"
                    f"  | extractions so far: {state['extractions_total']} ──\n",
                    flush=True,
                )

    # ── Collect all extractions ────────────────────────────────────────────────
    all_extractions: list[dict] = []
    for key, entry in state["processed"].items():
        parts = key.split("::")
        folder = parts[0] if len(parts) > 0 else "unknown"
        video  = parts[1] if len(parts) > 1 else "unknown"
        for ex in entry.get("extractions", []):
            ex.setdefault("source_folder", folder)
            ex.setdefault("source_video",  video)
            all_extractions.append(ex)

    print(f"\n{sep}")
    print("  COMPLETE")
    print(f"  Total extractions:   {len(all_extractions)}")
    print(f"  Total API calls:     {state['call_count']}")
    print(f"{sep}\n")

    write_output(all_extractions, state)
    print(f"Output written → {OUTPUT_DIR}\n")


# ══════════════════════════════════════════════════════════════════════════════
# OUTPUT WRITER
# ══════════════════════════════════════════════════════════════════════════════

def write_output(extractions: list[dict], state: dict) -> None:
    # Deduplicate by text (exact match)
    seen: set[str] = set()
    deduped: list[dict] = []
    for ex in extractions:
        t = ex.get("text", "").strip()
        if t and t not in seen:
            seen.add(t)
            deduped.append(ex)

    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # ── Per-stage files ────────────────────────────────────────────────────────
    for stage_num, filename in STAGE_FILES.items():
        stage_entries = [
            ex for ex in deduped if ex.get("nepq_stage", 0) == stage_num
        ]
        out_path = OUTPUT_DIR / filename
        lines = [
            f"# NEPQ Stage {stage_num} — {STAGE_NAMES.get(stage_num, 'General')} (Teaching Corpus)",
            "",
            f"Generated: {ts}",
            f"Total entries: {len(stage_entries)}",
            "",
        ]
        if stage_entries:
            # Group by type within stage
            by_type: dict[str, list[dict]] = {}
            for ex in stage_entries:
                by_type.setdefault(ex.get("type", "unknown"), []).append(ex)
            for etype in ALL_TYPES:
                if etype not in by_type:
                    continue
                lines.append(f"## {etype.replace('_', ' ').title()}")
                lines.append("")
                for ex in by_type[etype]:
                    lines.append(json.dumps({
                        "text":         ex.get("text", ""),
                        "type":         ex.get("type", ""),
                        "nepq_stage":   ex.get("nepq_stage", stage_num),
                        "context_note": ex.get("context_note", ""),
                        "tags":         ex.get("tags", []),
                        "source_video": ex.get("source_video", ""),
                        "source_folder": ex.get("source_folder", ""),
                    }, ensure_ascii=False))
                lines.append("")
        else:
            lines.append("*(no entries for this stage yet)*")
            lines.append("")
        out_path.write_text("\n".join(lines), encoding="utf-8")
        print(f"  Written: {filename}  ({len(stage_entries)} entries)", flush=True)

    # ── Special-type files ─────────────────────────────────────────────────────
    for etype, filename in TYPE_FILES.items():
        type_entries = [ex for ex in deduped if ex.get("type") == etype]
        out_path = OUTPUT_DIR / filename
        type_label = etype.replace("_", " ").title()
        lines = [
            f"# {type_label} (All Stages — Teaching Corpus)",
            "",
            f"Generated: {ts}",
            f"Total entries: {len(type_entries)}",
            "",
        ]
        if type_entries:
            # Group by NEPQ stage
            by_stage: dict[int, list[dict]] = {}
            for ex in type_entries:
                s = ex.get("nepq_stage", 0)
                by_stage.setdefault(s, []).append(ex)
            for s in sorted(by_stage.keys()):
                lines.append(f"## Stage {s} — {STAGE_NAMES.get(s, 'General')}")
                lines.append("")
                for ex in by_stage[s]:
                    lines.append(json.dumps({
                        "text":         ex.get("text", ""),
                        "type":         ex.get("type", ""),
                        "nepq_stage":   ex.get("nepq_stage", s),
                        "context_note": ex.get("context_note", ""),
                        "tags":         ex.get("tags", []),
                        "source_video": ex.get("source_video", ""),
                        "source_folder": ex.get("source_folder", ""),
                    }, ensure_ascii=False))
                lines.append("")
        else:
            lines.append("*(no entries yet)*")
            lines.append("")
        out_path.write_text("\n".join(lines), encoding="utf-8")
        print(f"  Written: {filename}  ({len(type_entries)} entries)", flush=True)

    # ── Teaching index ─────────────────────────────────────────────────────────
    total = len(deduped)

    # Counts by stage
    stage_counts = {s: sum(1 for ex in deduped if ex.get("nepq_stage", 0) == s)
                    for s in range(8)}

    # Counts by type
    type_counts = {t: sum(1 for ex in deduped if ex.get("type") == t) for t in ALL_TYPES}

    # Counts by source folder
    folder_counts: dict[str, int] = {}
    for ex in deduped:
        f = ex.get("source_folder", "unknown")
        folder_counts[f] = folder_counts.get(f, 0) + 1

    # Quality check flags
    stage_warnings: list[str] = []
    for s in range(1, 8):
        count = stage_counts.get(s, 0)
        target = 15 if s == 5 or s == 6 else 30  # lower bar for thin stages
        if count < target:
            stage_warnings.append(
                f"  ⚠  Stage {s} ({STAGE_NAMES[s]}): {count} entries — target ≥{target}"
            )

    gate_count = type_counts.get("gate_criteria", 0)
    gate_warn = f"  ⚠  gate_criteria: {gate_count} — target ≥15" if gate_count < 15 else ""

    index_lines = [
        "# Teaching Corpus — Index",
        "",
        f"Generated: {ts}",
        f"Total unique extractions: {total}",
        f"Total API calls: {state.get('call_count', 0)}",
        f"Processed chunks: {len(state.get('processed', {}))}",
        "",
        "## By NEPQ Stage",
        "",
    ]
    for s in range(8):
        name = STAGE_NAMES.get(s, "General")
        count = stage_counts.get(s, 0)
        target_note = " ← thin" if count < 30 and s > 0 else ""
        index_lines.append(f"| Stage {s} | {name} | {count} |{target_note}")
    index_lines.append("")

    index_lines += [
        "## By Extraction Type",
        "",
    ]
    for t in ALL_TYPES:
        count = type_counts.get(t, 0)
        index_lines.append(f"| {t} | {count} |")
    index_lines.append("")

    index_lines += [
        "## By Source Folder",
        "",
    ]
    for folder, count in sorted(folder_counts.items()):
        index_lines.append(f"| {folder} | {count} |")
    index_lines.append("")

    if stage_warnings or gate_warn:
        index_lines += [
            "## Quality Warnings",
            "",
        ]
        for w in stage_warnings:
            index_lines.append(w)
        if gate_warn:
            index_lines.append(gate_warn)
        index_lines.append("")

    index_lines += [
        "## Output Files",
        "",
    ]
    for s, fn in STAGE_FILES.items():
        count = stage_counts.get(s, 0)
        index_lines.append(f"- `{fn}` — {count} entries")
    for etype, fn in TYPE_FILES.items():
        count = type_counts.get(etype, 0)
        index_lines.append(f"- `{fn}` — {count} entries (cross-stage)")
    index_lines.append("")

    (OUTPUT_DIR / "_teaching-index.md").write_text(
        "\n".join(index_lines), encoding="utf-8"
    )
    print(f"  Written: _teaching-index.md  ({total} total unique extractions)", flush=True)


# ══════════════════════════════════════════════════════════════════════════════
# ENTRY POINT
# ══════════════════════════════════════════════════════════════════════════════

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Mine Jeremy Miner teaching content from NEPQ transcripts."
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=0,
        help="Only process first N sections (0 = no limit; use 5 for smoke test)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print first 5 chunks without making API calls",
    )
    args = parser.parse_args()
    asyncio.run(run(limit=args.limit, dry_run=args.dry_run))
